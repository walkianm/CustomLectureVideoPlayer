'''
Counts the number of times a section was watched
more than once. If a user watches a section 3 times,
that counts as 2 rewatches.
'''
...
def rm_main():
    ...
    # remove a single copy from each, leaving only duplicates
    watchProfile = [False] * size
    for item in data:
        start = 0
        stop = 0
        index = 0
        ...
        while index < len(item.get("events")):
            ...
            if started:
                while start <= stop:
                    watchProfile[start] = True
                    start = start + 1

            index = index + 1

        i = 0
        while i < len(watchProfile):
            if watchProfile[i]:
                timesWatched[i] = timesWatched[i] - 1
                watchProfile[i] = False
            i = i + 1
